...
public class QiuQiuPoppleAttackTwo : ActionState
{
    public QiuQiuPoppleAttackTwo(EActionType type, ActionMachine am, GameObject master, ActionState pnt = null, ActionState cld = null)
    ...
    {
        mAt.CrossFade("QQRAttackTwo", 0.05f);
        // mAt.Play("Idle");
    ...
    {
        mMaster.GetComponent<QiuQiuPopple>().weaPonRightHand.GetComponent<CapsuleCollider>().enabled = true;
        TimerMgr.Instance.OneShot("QQRRightAttackRectUnEnabled", 0.35f, QQRRightAttackRectUnEnabled);
    }
    void QQRRightAttackRectUnEnabled()
    {
        mMaster.GetComponent<QiuQiuPopple>().weaPonRightHand.GetComponent<CapsuleCollider>().enabled = false;
    }
